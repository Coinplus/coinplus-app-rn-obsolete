...
export const getBalances = addresses => {
    return fetch(`https://blockchain.info/multiaddr?active=${addresses.join("|")}`)
        .then(response => {
          return response.json();
        })
        .then(res => {
...
export const getBitCoinRate = () => {
  return fetch("https://api.coindesk.com/v1/bpi/currentprice.json")
      .then(response => {
        ...
        return Decimal(result.bpi.USD.rate.replaceAll(",",""))
      })
}
export const getBitcoinData = () => {
  return fetch("https://pro-api.coinmarketcap.com/v2/cryptocurrency/price-performance-stats/latest?symbol=BTC&time_period=24h&convert_id=2781",
      {method: 'GET',
        headers: {
